...
def get_cnn_model(input_shape, num_categories):
    ...
    for k in kernel_sizes:
        layer = Conv2D(filters=filters, kernel_size=k, padding='same', activation='relu', batch_size=50, input_shape=input_shape, kernel_regularizer=regularizers.l2(3))(input)
        # max-over-time pooling
        # the pool shape needs to be (1, sequence_length)
        pool = MaxPool2D(pool_size=(1, input_shape[1]), padding='same')(layer)
        layers.append(pool)
